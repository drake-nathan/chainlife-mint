...
export const fetchCurrentSupply = async (contract: Contract) => {
  const currentSupply = (await contract.methods
    .tokensMinted()
    .call()) as string;
...
export const checkIfEnsoUsed = async (
  contract: Contract,
  tokenId: number | string,
) => {
  const ensoUsed = (await contract.methods
    .preMintWithEnso(tokenId)
    .call()) as string;

  return ensoUsed === "1";
};

export const checkIfFocusUsed = async (
  contract: Contract,
  tokenId: number | string,
) => {
  const focusUsed = (await contract.methods
    .preMintWithFOCUS(tokenId)
    .call()) as string;
